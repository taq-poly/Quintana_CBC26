#!/usr/bin/env Rscript
# Imports Salmon quant.sf, runs DESeq2: female uterus (FU) vs female body
# wall (FBW) -- transcript-level (txOut = TRUE, no tx2gene collapsing).
...
samples$tissue <- factor(samples$tissue, levels = c("body_wall", "uterus"))
...
res <- results(dds, contrast = c("tissue", "uterus", "body_wall"), alpha = 0.05)
res_shrunk <- lfcShrink(dds, coef = "tissue_uterus_vs_body_wall", type = "apeglm")
...
write_csv(res_df, "results/deseq2/DE_uterus_vs_bodywall.csv")

sig <- subset(res_df, !is.na(padj) & padj < 0.05 & abs(log2FoldChange) > 1)
write_csv(sig, "results/deseq2/DE_uterus_vs_bodywall_significant.csv")
...
p_pca <- ggplot(pca_data, aes(PC1, PC2, color = tissue, label = name)) +
  geom_point(size = 4) +
  geom_text(vjust = -1, size = 3) +
  xlab(paste0("PC1: ", pct_var[1], "% variance")) +
  ylab(paste0("PC2: ", pct_var[2], "% variance")) +
  theme_bw() +
  ggtitle("PCA: D. immitis uterus vs body wall (VST counts)")
...
p_volcano <- ggplot(res_df, aes(x = log2FoldChange, y = -log10(pvalue), color = sig)) +
  ...
  theme_bw() +
  labs(title = "Uterus vs body wall: volcano plot",
